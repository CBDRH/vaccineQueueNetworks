...
# queueDefault1 <- list()
# for(i in 1:20){
#   queueDefault1[[i]] <- arenaModel(arrivals = arrivals(120, 60, 0, 5, 0.02), nRsi = 4, nEnt = 8, nReg = 12, nAss = 8, nVac = 10)
# }
# 
# # Default queue plots
...
# queueDefault2 <- list()
# for(i in 1:20){
#   queueDefault2[[i]] <- gpModel(arrivals = arrivals(4, 10, -3, 1, 0.02), nRsi = 2, nReg = 2, nVac = 4)
# }
# # Default queue plots
...
# infoThroughPut1 <- queueTimes1 %>% filter(station == 'Total') %>% group_by(iter) %>% summarise(n = max(id)) %>% summarise(mean = median(n))
# save(infoThroughPut1, file = here::here('queueSimTool/www/infoThroughPut1'))
# 
# ## Median processing time
# infoProcessTime1 <- queueTimes1 %>% filter(station == 'Total') %>% summarise(mean = median(mins))
# save(infoProcessTime1, file = here::here('queueSimTool/www/infoProcessTime1'))
# 
# ## Vaccinations per day
# infoThroughPut2 <- queueTimes2 %>% filter(station == 'Total') %>% group_by(iter) %>% summarise(n = max(id)) %>% summarise(mean = median(n))
# save(infoThroughPut2, file = here::here('queueSimTool/www/infoThroughPut2'))
# 
# ## Median processing time
# infoProcessTime2 <- queueTimes2 %>% filter(station == 'Total') %>% summarise(mean = median(mins))
# save(infoProcessTime2, file = here::here('queueSimTool/www/infoProcessTime2'))
# 
# 
...
#                   ggplot(aes(x = 1, y = n)) +
#                   geom_boxplot() +
#                   scale_y_continuous('Number of vaccinations delivered', breaks = scales::pretty_breaks()) +
...
#                   theme(text = element_text(size=14))
# 
# save(processingP1, file = here::here('queueSimTool/www/processingP1'))
# 
# 
# 
# 
...
#         text = element_text(size=14))
# 
# save(throughputP2, file = here::here('queueSimTool/www/throughputP2'))
# 
...
#   theme(text = element_text(size=14))
# 
# save(utilisationP2, file = here::here('queueSimTool/www/utilisationP2'))
# 
...
#   theme(text = element_text(size=14))
# 
# save(processingP2, file = here::here('queueSimTool/www/processingP2'))
